...
// --- Main Component -----------------------------------------------------------------------------------------
const DetailCard = ({ title, data, countries = [], PortCharges = [], ports = [], config = [], editable, orderType, section, vehicles = null, portSizeId }) => {
    ...
    editable = section == 'quotation' ? (orderType == 'Single' && editable ? true : false) : editable;
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            const response = await authAPI.orderDetailUpdate(payload);
        ...
        <div className="vehicle-list-wrapper">
            {vehiclesData.map((item, itemIndex) => (
                <div key={`vehicle-${item.id || itemIndex}`} className="dt-info-row">
                    <DataDisplayItem
                        label={`Vehicle Cost ${orderType !== 'Single' ? '(' + item.vehicleNo + ')' : ''}`}

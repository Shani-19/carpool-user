...
export default function SingleBike({ BikeItem, relatedBikes = [] }) {
  const { user } = useAuth();
  ...
  const urlType = pathname?.split('/').filter(Boolean)[0] || 'Bikes';
  const [isFav, setIsFav] = useState(false);

  useEffect(() => {
    if (BikeItem?.slug) {
      setIsFav(isFavourite(BikeItem.slug, urlType));
    }
  }, [BikeItem?.slug, urlType]);

  const handleFavourite = (e) => {
    e.preventDefault();
    if (!BikeItem) return;
    const newState = toggleFavourite(BikeItem, urlType);
    setIsFav(newState);
  };
  ...
  useEffect(() => {
    if (BikeItem?.vin) {
      ...
      api.post('/report-check', { vin: BikeItem.vin })
        .then(res => {
  ...
  const images = BikeItem?.images?.length > 0 ? BikeItem.images : [BikeItem?.main_image || "/images/resource/inventory1-6.png"];
  const imageBase = process.env.NEXT_PUBLIC_BIKES_IMG_SRC_S3 || "https://media.carpoolkr.com/assets/car/cars";
  ...
  const detailedSpecs = [
    { label: "Overall Length", value: BikeItem?.length ? `${BikeItem.length} mm` : null },
    { label: "Overall Width", value: BikeItem?.width ? `${BikeItem.width} mm` : null },
    { label: "Overall Height", value: BikeItem?.height ? `${BikeItem.height} mm` : null },
    { label: "Seating Capacity", value: BikeItem?.passenger ? `${BikeItem.passenger} Persons` : null },
    { label: "Fuel Type", value: BikeItem?.fuel_type },
    { label: "Transmission", value: BikeItem?.transmission },
    { label: "Engine Volume", value: BikeItem?.engine_volume ? `${fmtNumber(BikeItem.engine_volume)} cc` : null },
    { label: "Engine Power", value: BikeItem?.engine_power ? `${BikeItem.engine_power} hp` : null },
    { label: "Color", value: BikeItem?.color },
    { label: "Registration Date", value: BikeItem?.registration_date },
    { label: "Category", value: BikeItem?.category },
  ...
  return (
    <section className="inventory-section v1 layout-radius">
      <div className="boxcar-container">
        <div className="boxcar-title-three">
          <ul className="breadcrumb">
            ...
            <li><span>Bikes for Sale</span></li>
          </ul>

          <h2>{BikeItem?.name || "Bike Name"}</h2>

          <div className="text" style={{ color: "gray" }}>
            {BikeItem?.registration_date} ({BikeItem?.year}), {BikeItem?.fuel_type}, {BikeItem?.transmission}, {BikeItem?.passenger ? `${BikeItem?.passenger} Seater` : ""}
          </div>
          ...
          <div className="content-box">
            ...
            <h3 className="title">${fmtNumber(BikeItem?.final_price || BikeItem?.price)}</h3>
            <span>
              ...
              &nbsp;&nbsp; {BikeItem?.status?.toLowerCase() === 'sale' && BikeItem?.booking_status?.toLowerCase() === 'sale' ? 'Sale' : (BikeItem?.status !== 'Sale' ? 'Sold' : 'Reserved')}
            </span>
          </div>
        </div>
          ...
          <div className="inspection-column v2 col-xl-8 col-lg-12 col-md-12 col-sm-12">
            <div className="inner-column">
                  ...
                  <div className="image-column wrap-gallery-box">
                    ...
                    <div className="content-box">
                      <ul className="video-list">
                        ...
                        <li>
                          <a href="#"><img src="/images/resource/eye.svg" alt="" />{BikeItem?.view || 0}</a>
                        </li>
                      </ul>
                    </div>
                  </div>
              ...
              <div className="overview-sec-two v2">
                <ul className="list">
                  {BikeItem?.odometer && BikeItem.odometer !== "-" && (
                    <li>
                      <img src="/images/resource/insep2-1.svg" alt="" />
                      <span>Kilometer</span>
                      <small>{fmtNumber(BikeItem.odometer)}</small>
                    </li>
                  )}
                  {BikeItem?.fuel_type && BikeItem.fuel_type !== "-" && (
                    <li>
                      <img src="/images/resource/insep2-2.svg" alt="" />
                      <span>Fuel Type</span>
                      <small>{BikeItem.fuel_type}</small>
                    </li>
                  )}
                  {BikeItem?.transmission && BikeItem.transmission !== "-" && (
                    <li>
                      <img src="/images/resource/insep2-3.svg" alt="" />
                      <span>Transmission</span>
                      <small>{BikeItem.transmission}</small>
                    </li>
                  )}
                  {BikeItem?.passenger && BikeItem.passenger !== "-" && (
                    <li>
                      <img src="/images/resource/insep2-4.svg" alt="" />
                      <span>Seating</span>
                      <small>{`${BikeItem.passenger} Persons`}</small>
                    </li>
                  )}
                  {BikeItem?.make_name && BikeItem.make_name !== "-" && (
                    <li>
                      <img src="/images/resource/insep2-5.svg" alt="" />
                      <span>Manufacturer</span>
                      <small>{BikeItem.make_name}</small>
                    </li>
                  )}
                </ul>
              </div>
              ...
              <div className="description-sec v2">
                ...
                <div className="text" dangerouslySetInnerHTML={{ __html: BikeItem?.description || "No description available." }} />
          ...
          <div className="side-bar-column v2 v3 v4 col-xl-4 col-lg-12 col-md-12 col-sm-12">
            <div className="inner-column">
              <div className="overview-box">
                <h4 className="title">Bike Overview</h4>
                <ul className="list v2">
                  <li><span><img src="/images/resource/insep1-5.svg" alt="" />Manufacturer</span>{BikeItem?.make_name || "-"}</li>
                  <li><span><img src="/images/resource/insep1-3.svg" alt="" />Model</span>{BikeItem?.model_name || "-"}</li>
                  <li><span><img src="/images/resource/insep1-4.svg" alt="" />Year</span>{BikeItem?.year || "-"}</li>
                  <li><span><img src="/images/resource/insep1-11.svg" alt="" />Color</span>{BikeItem?.color || "-"}</li>
                  <li><span><img src="/images/resource/insep1-7.svg" alt="" />Seats</span>{BikeItem?.passenger ? `${BikeItem.passenger}-seat` : "-"}</li>
                  <li><span><img src="/images/resource/insep1-1.svg" alt="" />Engine Power</span>{BikeItem?.engine_power ? `${BikeItem.engine_power} hp` : "-"}</li>
                  <li><span><img src="/images/resource/insep1-2.svg" alt="" />Engine No.</span>{BikeItem?.engine_no || "-"}</li>
                  <li><span><img src="/images/resource/insep1-12.svg" alt="" />Part No</span>{BikeItem?.vin || "-"}</li>
                </ul>
              </div>
              ...
              <div className="form-box v2 mt-4">
                <ShippingCalculator vehicleItem={BikeItem} displayPrice={BikeItem?.final_price || BikeItem?.price} />
              </div>
      ...
      <ModalVideo
        ...
        isOpen={isOpen}
        videoId={BikeItem?.embed_code || ""}
        onClose={() => setOpen(false)}
      />
      <ShareModal isOpen={isShareModalOpen} onClose={() => setIsShareModalOpen(false)} vehicleData={BikeItem} />
            ...
            >
              ...
              <div className="modal-body p-4">
                <p className="text-muted mb-3" style={{ fontSize: '14px', lineHeight: '1.5' }}>
                  Please confirm your details and vehicle information to request a professional inspection report for <strong>{BikeItem?.name}</strong>.
                </p>
                ...
                <div className="bg-light p-3 rounded-3 mb-4" style={{ fontSize: '13px', border: '1px solid #e2e8f0' }}>
                  <div className="row g-3">
                    ...
                    <div className="col-12 col-md-6">
                      ...
                      <div className="d-flex flex-column gap-1">
                        <div><span className="text-muted">VIN:</span> <strong className="text-dark">{BikeItem?.vin || "-"}</strong></div>
                        <div><span className="text-muted">Plate:</span> <strong className="text-dark">{BikeItem?.plate_no || "-"}</strong></div>
                        <div><span className="text-muted">Transmission:</span> <strong className="text-dark">{BikeItem?.transmission || "-"}</strong></div>
                        <div><span className="text-muted">Type:</span> <strong className="text-dark">{BikeItem?.vehicle_type || BikeItem?.category || "-"}</strong></div>
                      </div>
                    </div>
                  </div>
                </div>

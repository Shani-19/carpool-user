...
export default function RelatedParts({ Parts = [] }) {
  const imageBase = "https://media.carpoolkr.com/assets/part/thumbnail/";
    ...
    <section className="cars-section-three">
      <div className="boxcar-container">
        <div className="boxcar-title wow fadeInUp">
          ...
          <Link href="/parts" className="btn-title">
            View All
        ...
        <Slider
          slidesToScroll={1}
          slidesToShow={4}
          infinite={Parts.length > 4}
          responsive={[
            { breakpoint: 1600, settings: { slidesToShow: 4, infinite: Parts.length > 4 } },
            { breakpoint: 1300, settings: { slidesToShow: 3, infinite: Parts.length > 3 } },
            { breakpoint: 991, settings: { slidesToShow: 2, infinite: Parts.length > 2 } },
            { breakpoint: 767, settings: { slidesToShow: 1, infinite: Parts.length > 1 } },
          ]}
          arrows
          className="row car-slider-three wow fadeInUp"
        >
          {Parts.map((Part, index) => {
            const imageUrl = Part.main_image?.startsWith("http") ? Part.main_image : `${imageBase}${Part.main_image}`;
            return (
              <div
                key={index}
                className="car-block-three col-lg-3 col-md-6 col-sm-12"
                onClick={() => window.open(`/parts/${Part.slug}`, '_blank')}
                ...
                <div className="inner-box">
                  <div className="image-box">
                    <div className="slider-thumb">
                      <div className="image">
                        <Link href={`/parts/${Part.slug}`} target="_blank">
                          <Image alt={Part.name} src={imageUrl} width={329} height={220} style={{ objectFit: "cover" }} />
                        </Link>
                      </div>
                    </div>
                    <Link href={`/parts/${Part.slug}`} target="_blank" title="" className="icon-box">
                  ...
                  <div className="content-box">
                    <h6 className="title"><Link href={`/parts/${Part.slug}`} target="_blank">{Part.name}</Link></h6>
                    <ul className="list-info mt-3 justify-content-between">
                      <li><i className="flaticon-speedometer" /> {Part.category || "-"}</li>
                      <li><i className="flaticon-gasoline-pump" /> {Part.make_name || Part.make || "-"}</li>
                      <li><i className="flaticon-gearbox" /> {Part.model_name || Part.model || "-"}</li>
                    </ul>
                    <div className="btn-box">
                      <span>${fmtNumber(Part.final_price || (Part.price - (Part.discount_price || 0)))}</span>
                      <Link href={`/parts/${Part.slug}`} target="_blank" className="details">
                        View Details
                        <svg xmlns="http://www.w3.org/2000/svg" width={14} height={14} viewBox="0 0 14 14" fill="none">
